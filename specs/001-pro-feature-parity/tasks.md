...
- [X] T032 [US4] Forward-reference finalization: `CorrectEmbeddedIssueLinks` now reports `unresolved`; `Agent` records those work items and `Agent.FinalizeEmbeddedLinks` (called by `ImportCommandLine` after the import loop) re-fetches them and re-runs correction via `WitClientUtils.FinalizeEmbeddedIssueLinks` (journal now complete). +2 tests in `EmbeddedLinkFinalizationTests.cs`
