        ...
        private bool ExecuteMigration(CommandOption token, CommandOption url, CommandOption configFile, bool forceFresh, CommandOption continueOnCritical)
        {
            ...
            try
            {
                ...
                var settings = new Settings(url.Value(), config.TargetProject, token.Value())
                {
                    ...
                    CorrectEmbeddedLinks = config.CorrectEmbeddedLinks
                };

                // Load sprint metadata written by jira-export (sprint dates/state for ADO iteration nodes)
                var sprintMetaPath = Path.Combine(config.Workspace, "sprint-metadata.json");
                if (File.Exists(sprintMetaPath))
                {
                    var sprintDates = JsonConvert.DeserializeObject<Dictionary<string, SprintDateInfo>>(File.ReadAllText(sprintMetaPath));
                    if (sprintDates != null)
                    {
                        settings.SprintDates = sprintDates;
                        Logger.Log(LogLevel.Info, $"Loaded sprint metadata for {sprintDates.Count} sprint(s) from '{sprintMetaPath}'.");
                    }
                }
