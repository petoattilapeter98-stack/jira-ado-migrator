...
namespace JiraExport
{
    ...
    public class JiraItem
    {
        ...
        private static readonly ConcurrentDictionary<string, SprintMetadata> _sprintMetadataCache =
        ...
        private static Dictionary<string, object> ExtractFields(string key, JObject remoteIssue, IJiraProvider jira)
        {
            ...
            foreach (var prop in remoteFields.Properties())
            {
                ...
                else if (type == JTokenType.Array && prop.Value.Any())
                {
                    ...
                    // Cache sprint metadata (start/end/state) if this array contains sprint objects
                    foreach (var token in prop.Value.Children<JObject>())
                    {
                        var sprintName = token["name"]?.ToString();
                        if (!string.IsNullOrEmpty(sprintName) && (token["startDate"] != null || token["state"] != null))
                        {
                            DateTime? startDate = null, endDate = null;
                            if (DateTime.TryParse(token["startDate"]?.ToString(), out var sd)) startDate = sd;
                            var endStr = token["completeDate"]?.ToString() ?? token["endDate"]?.ToString();
                            if (DateTime.TryParse(endStr, out var ed)) endDate = ed;
                            _sprintMetadataCache[sprintName] = new SprintMetadata
                            {
