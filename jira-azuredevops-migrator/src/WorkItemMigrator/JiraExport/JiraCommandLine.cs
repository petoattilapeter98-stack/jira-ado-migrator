    ...
    public class JiraCommandLine
    {
        ...
        private bool ExecuteMigration(CommandOption user, CommandOption password, CommandOption token, CommandOption url, CommandOption configFile, bool forceFresh, CommandOption continueOnCritical)
        {
            ...
            try
            {
                ...
                // Write sprint metadata (start/end/state) so the importer can set iteration dates in ADO
                var sprintMeta = JiraItem.GetSprintMetadata();
                if (sprintMeta.Count > 0)
                {
                    var sprintMetaPath = Path.Combine(migrationWorkspace, "sprint-metadata.json");
